...
WorkingDirectory=/home/${server2_user}/app/zookeeper
Restart=always 
RestartSec=0s
ExecStart=/home/${server2_user}/app/zookeeper/bin/zkServer.sh start
ExecStop=/home/${server2_user}/app/zookeeper/bin/zkServer.sh stop 
[Install] 
...
var server_properties_data = `broker.id=2
advertised.listeners=PLAINTEXT://${server2_ip}:9092
...
WorkingDirectory=/home/${server2_user}/app/kafka
Restart=no
RestartSec=0s
ExecStart=/home/${server2_user}/app/kafka/bin/kafka-server-start.sh /home/${server2_user}/app/kafka/config/server.properties
ExecStop=/home/${server2_user}/app/kafka/bin/kafka-server-stop.sh
[Install]
WantedBy=multi-user.target
`;
...
var mariadb_fileName='server.cnf';
var mariadb_encoding = 'utf8';
var mariadb_data = `[server]
[mysqld]
[galera]
wsrep_on=ON
...
[mariadb-10.5]
`;
fs.writeFile(url+mariadb_fileName,mariadb_data,mariadb_encoding,function(err){
  if(err) console.log('Error'+err);
    else console.log("쓰기완료");
});
